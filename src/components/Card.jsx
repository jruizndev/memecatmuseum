...
const Card = ({ meme, handleDelete, handleFlip, flipped }) => {
  return (
    <div
      key={meme.id}
      className={`group relative max-w-[350px] min-h-[300px] p-2 transition-all ease-in-out`}
      style={{ perspective: '1000px' }}
    >
      <div className="relative w-full h-full [transform-style:preserve-3d] transition-transform rounded-5xl ">
        {/* Parte frontal */}
        <div
          className={`absolute w-full h-full [backface-visibility:hidden] transition-transform transform rounded-3xl p-2 z-10 ${flipped ? 'hidden' : ''}`}
        >
          <div className="relative w-full max-w-[350px] min-h-[350px] aspect-[4/3] flex justify-center items-center overflow-hidden rounded-lg group-hover:bg-white transition-colors">
            {/* Imagen solo en la parte frontal */}
            <img
              src={meme.image}
              alt={meme.description}
              className="max-w-full[300px] max-h-[300px] min-h-[200px] w-auto h-auto object-contain rounded-3xl p-2"
            />
            {/* Botones en la parte baja de la parte frontal */}
            <div className="absolute bottom-0 left-0 right-0 p-2 flex justify-evenly opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {/* Parte trasera */}
        <div className={`absolute min-h-[400px] bg-white [backface-visibility:hidden] transition-transform transform rotate-y-180 rounded-3xl p-2 z-10 ${flipped ? '' : 'hidden'}`}>
          <div className="relative w-full h-full">
            {/* Imagen oscurecida */}
            <img
              src={meme.image}
              alt={meme.description}
              className="absolute inset-0 object-cover rounded-3xl opacity-20"
            />
            <div className="relative min-h-[350px] p-4 z-10 bg-black/60 text-white">
              <h2 className="text-md font-semibold text-center mb-2">{meme.name}</h2>
              <p className="text-sm text-center">Description: {meme.description}</p>
              <p className="text-sm text-center">Likes: {meme.likes}</p>
              <p className="text-sm text-center">Fecha: {meme.date}</p>
            </div>
          </div>
          {/* Botones en la parte baja de la parte trasera */}
